import {getSubCategory,updateSubCategory} from '../../../../services/Sub_category_services'
...
class Edit_sub_category_content extends Component{
	state = {
		id : '',
		name : '',
		description : '',
	...
	getSubCategoryInfo = async () => {
		let response = await getSubCategory(this.props.match.params.id)
		this.setState({
			id : response.data.id,
	...
	submitForm = async (e) => {
		...
		const response = await updateSubCategory(this.state.id,dataToStore)
		if(response.success){
			this.setState({
				id : '',
				name : '',
		...
		return(
			<div className="fix ninty_eight_percent div_mid bg_white pt_10 pr_10 pb_10 pl_10 border_box">
				<div className="fix full">
					<p className="fs_24 lh_34 font_bold text_dark_ash floatleft">Edit Sub Category</p>
				...
				<div className="fix full mt_20 mb_20">
					<p className="fs_14 lh_20 text_dark_ash">Edit Sub Category</p>
				</div>
				<Border />
									...
									<div className="fix full mt_10">
										{this.state.categories_to_show.map((category, index) => (
											<div className="fix full mb_5" key={index}>
												...
												checked={(this.state.categories.split(',').includes(category.id.toString()))}/>
...
export default withRouter(Edit_sub_category_content)
...
import {
	...
	getDataByNameFromLocalList} from '../../../../services/Color_services'
import Button from '../../../common_elements/Button'
import { confirmAlert } from 'react-confirm-alert'; // Import
import 'react-confirm-alert/src/react-confirm-alert.css'; // Import css
class View_colors_content extends Component{
	state = {
		colors : [],
		temporary_colors : [],
	...
	getColors = async (callback = () => {}) => {
		...
		this.setState({
			...
			temporary_colors : response.data
		})
		callback()
	}
	deleteDataFromLocal = (id) => {
		let colors = deleteDataByIdFromLocalList('id',this.state.selectedColorId,this.state.colors)
		this.setState({
			colors,
	...
	searchByName = () => {
		let foundColors = getDataByNameFromLocalList(this.state.search_data, this.state.colors)
		this.setState({
			temporary_colors : foundColors
		})
	}
						...
						<div className='fix w_300 bt_1 br_1 bb_1 bl_1 border_solid border_dark_ash'>
							...
							<div className="fix full pt_5 pr_10 pb_5 pl_5 border_box">
								...
								<div className="fix full mt_20 floatright">
									...
									buttonFloat="floatright"
									buttonFunction={() => {
										deleteColor(this.state.selectedColorId)
										this.getColors(() => {onClose()})
										this.setState({
											selectedColorId : null
										})
		...
		return(
			<div className="fix ninty_eight_percent div_mid bg_white pt_10 pr_10 pb_10 pl_10 border_box">
				<div className="fix full">
					<p className="fs_24 lh_34 font_bold text_dark_ash floatleft">View Colors</p>
					<Link to='/manage/colors/add_color'>
						<button className="bg_brown2 fs_14 pt_10 pb_10 pr_16 pl_16 font_bold floatleft text_white border_none ml_20 cursor_pointer">Add Color</button>
					</Link>
				</div>
				<div className="fix full mt_20 mb_20">
					<p className="fs_14 lh_20 text_dark_ash">View and Manage Colors</p>
				</div>
				...
				<div className="fix full">
					<table className="full text_dark_ash fs_14 lh_26">
						...
						<tbody className="textcenter">
							{
								(this.state.temporary_colors.length > 0) ?
									this.state.temporary_colors.map((color, index) => (
										<tr key={index}>
											<td><input type="checkbox"/></td>
											<td>{index+1}</td>
											<td>{color.name}</td>
											<td>{(color.is_active == 0) ? 'Inactive' : 'Active'}</td>
											<td>{moment(color.created_at).format('YYYY-MM-DD')}</td>
											<td>
												<Link to={`/manage/colors/edit_color/${color.id}`}>
...
export default View_colors_content

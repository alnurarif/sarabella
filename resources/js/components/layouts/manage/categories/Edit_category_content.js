...
import {getCategory,updateCategory} from '../../../../services/Category_services'
import {makeAnObjectOfLaravelError} from '../../../../functions/Error_functions'
import Button from '../../../common_elements/Button'
class Edit_category_content extends Component{
	state = {
		id : '',
		name : '',
		has_url : '0',
	...
	getCategoryInfo = async () => {
		let response = await getCategory(this.props.match.params.id)
		this.setState({
			id : response.data.id,
			name : response.data.name,
			has_url : response.data.has_url.toString(),
			selected_has_url : response.data.has_url.toString(),
	...
	submitForm = async (e) => {
		...
		const response = await updateCategory(this.state.id,dataToStore)
		if(response.success){
			this.setState({
				id : '',
				name : '',
		...
		return(
			<div className="fix ninty_eight_percent div_mid bg_white pt_10 pr_10 pb_10 pl_10 border_box">
				<div className="fix full">
					<p className="fs_24 lh_34 font_bold text_dark_ash floatleft">Edit Category</p>
				...
				<div className="fix full mt_20 mb_20">
					<p className="fs_14 lh_20 text_dark_ash">Edit Category</p>
				</div>
				<Border />
...
export default withRouter(Edit_category_content)
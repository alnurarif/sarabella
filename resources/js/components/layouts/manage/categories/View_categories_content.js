...
import {
	...
	getDataByNameFromLocalList} from '../../../../services/Category_services'
import Button from '../../../common_elements/Button'
import { confirmAlert } from 'react-confirm-alert'; // Import
import 'react-confirm-alert/src/react-confirm-alert.css'; // Import css
class View_categories_content extends Component{
	state = {
		categories : [],
	...
	getCategories = async (callback = () => {}) => {
		...
		this.setState({
			...
			temporary_categories : response.data
		})
		callback()
	}
	deleteDataFromLocal = (id) => {
		let categories = deleteDataByIdFromLocalList('id',this.state.selectedCategoryId,this.state.categories)
		this.setState({
			categories,
			temporary_categories : categories
		})
	}
	searchByName = () => {
		let foundCategories = getDataByNameFromLocalList(this.state.search_data, this.state.categories)
		this.setState({
			temporary_categories : foundCategories
		})
	}
						...
						<div className='fix w_300 bt_1 br_1 bb_1 bl_1 border_solid border_dark_ash'>
							...
							<div className="fix full pt_5 pr_10 pb_5 pl_5 border_box">
								...
								<div className="fix full mt_20 floatright">
									...
									buttonFloat="floatright"
									buttonFunction={() => {
										deleteCategory(this.state.selectedCategoryId)
										this.getCategories(() => {onClose()})
										this.setState({
											selectedCategoryId : null
										})
		...
		return(
			<div className="fix ninty_eight_percent div_mid bg_white pt_10 pr_10 pb_10 pl_10 border_box">
				<div className="fix full">
					<p className="fs_24 lh_34 font_bold text_dark_ash floatleft">View Categories</p>
					<Link to='/manage/categories/add_category'>
						<button className="bg_brown2 fs_14 pt_10 pb_10 pr_16 pl_16 font_bold floatleft text_white border_none ml_20 cursor_pointer">Add Category</button>
					</Link>
				</div>
				<div className="fix full mt_20 mb_20">
					<p className="fs_14 lh_20 text_dark_ash">View and Manage Categories</p>
				</div>
				...
				<div className="fix full">
					<table className="full text_dark_ash fs_14 lh_26">
						...
						<tbody className="textcenter">
							{
								(this.state.temporary_categories.length > 0) ?
									this.state.temporary_categories.map((category, index) => (
										<tr key={index}>
											<td><input type="checkbox"/></td>
											<td>{index+1}</td>
											<td>{category.name}</td>
											<td>{(category.is_active == 0) ? 'Inactive' : 'Active'}</td>
											<td>{moment(category.created_at).format('YYYY-MM-DD')}</td>
											<td>
												<Link to={`/manage/categories/edit_category/${category.id}`}>
												...
												<p className="cursor_pointer ml_2 mr_2 fs_14 textcenter font_bold text_dark_ash display_inline_block" onClick={() => this.deleteConfirmation(category.id)}><i className="fas fa-trash-alt"></i></p>
...
export default View_categories_content

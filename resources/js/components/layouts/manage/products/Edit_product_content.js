import {checkColorExistsInAvailableColor,updateProduct} from '../../../../services/Product_services'
...
class Edit_product_content extends Component{
	...
	methods = {
		getProductById : async () => {
			...
			this.methods.setProduct(productInfo.data)
			this.methods.setProductDetails(productInfo.data)
		},
		setProduct : (data) => {
			this.setState({
				product : data
		...
		setProductDetails : (data) => {
			console.log(data)
			let selected_colors = []
			let warrenty_options = []
			let images = []
			data.colors.map((selected_single_color,selected_index) => {
				this.state.product_available_colors.map(async (available_single_color,index) => {
					if(selected_single_color.pivot.color_id == available_single_color.id){

						await fetch(`${process.env.MIX_APP_URL}/uploads/products_colors/${selected_single_color.pivot.sample_image}`)
						.then( response => response.blob() )
						.then( blob =>{
						    var reader = new FileReader() ;
						    const scope = this
						    reader.onload = function(){ 
								available_single_color.sample_image = this.result
								available_single_color.has_free_sample = selected_single_color.pivot.has_free_sample
								selected_colors.push(available_single_color)
								// scope.setState(prevState => ({
								//   images: [...prevState.images, this.result]
								// }))
						    } ;
			...
			data.warrenty_options.map((single_warrenty,index) => {
				let single_warrenty_object = {
					warrenty_details : single_warrenty.warrenty_option_detail,
					warrenty_price : single_warrenty.warrenty_option_price
				}
				warrenty_options.push(single_warrenty_object)
			})
			if(data.images !== undefined && data.images.length > 0){
				data.images.map(async (single_image,index) => {
					await fetch(`${process.env.MIX_APP_URL}/uploads/products/${single_image.image_name}`)
					.then( response => response.blob() )
					.then( blob =>{
					    var reader = new FileReader() ;
					    const scope = this
			...
			this.setState({
				...
				product_main_category : String(data.main_category),
				product_sub_category : String(data.sub_category),
				unit_price :  String(data.unit_price),
				offer_price :  String(data.offer_price),
				offer_price_type : String(data.offer_price_type),
				delivery_time : String(data.delivery_time),
				delivery_charge : (data.delivery_charge === null) ? '' : String(data.delivery_charge),
				selected_colors,
				samples_available_or_not : String(data.sample_available),
				selected_sample_available_or_not : String(data.sample_available),
				short_features : JSON.parse(data.short_features),
				mount_type_inside : data.mount_type_inside,
				mount_type_outside : data.mount_type_outside,
				width_inch : String(data.width_inch),
				width_8ths : String(data.width_8ths),
				height_inch : String(data.height_inch),
				height_8ths : String(data.height_8ths),
				minimum_quantity : String(data.minimum_quantity),
				maximum_quantity : String(data.maximum_quantity),
				total_product_number : String(data.total_product_number),
				has_features : String(data.has_features),
				product_info : '',
				measure_and_install : '',
				shipping_and_production : '',
				warrenty_options,
				...
				product_info_description : data.product_info_description,
				product_info_feature_text : '',
				product_info_option_text : '',
				product_info_motorization_text : '',
				product_info_recommendation_text : '',
				product_info_features : JSON.parse(data.product_info_features),
				product_info_options : JSON.parse(data.product_info_options),
				product_info_motorizations : JSON.parse(data.product_info_motorizations),
				product_info_recommendations : JSON.parse(data.product_info_recommendations),
				...
				specification_widths : JSON.parse(data.specification_widths),
				specification_heights : JSON.parse(data.specification_heights),
				specification_min_inside_mount_depths : JSON.parse(data.specification_min_inside_mount_depths),
				specification_min_flush_mount_depths : JSON.parse(data.specification_min_flush_mount_depths),
				specification_headrail_dimensions : JSON.parse(data.specification_headrail_dimensions),
				specification_min_outside_mount_spaces : JSON.parse(data.specification_min_outside_mount_spaces),
				specification_min_flush_inside_mount_depths : JSON.parse(data.specification_min_flush_inside_mount_depths),
				specification_min_outside_mount_depths : JSON.parse(data.specification_min_outside_mount_depths),

				shipping_and_production_text : '',
				shipping_and_productions : JSON.parse(data.shipping_and_production),
				shipping_policy : data.shipping_policy,
				shipping_policy_note: data.shipping_policy_note,


				is_cordless_lift_active : String(data.is_cordless_lift_active), 
				is_continuous_loop_active : String(data.is_continuous_loop_active), 
				is_motorization_active : String(data.is_motorization_active), 
				cordless_lift_price :  (data.cordless_lift_price === null) ? '' : String(data.cordless_lift_price), 
				ccl_price :  (data.ccl_price === null) ? '' : String(data.ccl_price), 
				motorization_price :  (data.motorization_price === null) ? '' : String(data.motorization_price), 
				is_ccl_lift_chain_location_active : String(data.is_ccl_lift_chain_location_active),
				is_ccl_type_of_chain_active : String(data.is_ccl_type_of_chain_active), 

				ccl_chain_type_and_price :  JSON.parse(data.ccl_chain_type_and_price),  
				is_motorization_remote_control_active : String(data.is_motorization_remote_control_active), 
				motorization_remote_control_price :  (data.motorization_remote_control_price === null) ? '' : String(data.motorization_remote_control_price),
				is_motorization_wifi_active : String(data.is_motorization_wifi_active),
				motorization_wifi_price :  (data.motorization_wifi_price === null) ? '' : String(data.motorization_wifi_price),
				is_standard_roll_active : String(data.is_standard_roll_active),
				is_reverse_roll_active : String(data.is_reverse_roll_active),
				standard_roll_price : (data.standard_roll_price === null) ? '' : String(data.standard_roll_price),
				reverse_roll_price :  (data.reverse_roll_price === null) ? '' : String(data.reverse_roll_price),
				is_exposed_roll_active : String(data.is_exposed_roll_active),
				is_cassette_roll_active : String(data.is_cassette_roll_active),
				exposed_roll_price :  (data.exposed_roll_price === null) ? '' : String(data.exposed_roll_price),
				cassette_roll_price : (data.cassette_roll_price === null) ? '' : String(data.cassette_roll_price),
				is_plain_hem_active : String(data.is_plain_hem_active),
				is_wave_hem_active : String(data.is_wave_hem_active),
				is_scallop_hem_active : String(data.is_scallop_hem_active),
				is_rounded_hem_active : String(data.is_rounded_hem_active),
				plain_hem_price :  (data.plain_hem_price === null) ? '' : String(data.plain_hem_price),
				wave_hem_price :  (data.wave_hem_price === null) ? '' : String(data.wave_hem_price),
				scallop_price :  (data.scallop_price === null) ? '' : String(data.scallop_price),
				rounded_hem_price : (data.rounded_hem_price === null) ? '' : String(data.rounded_hem_price)
			})
		}
	}
	...
	submitForm = async (e) => {
		...
		const response = await updateProduct(dataToStore.id,dataToStore)
		...
		return(
			<div className="fix ninty_eight_percent div_mid bg_white pt_10 pr_10 pb_10 pl_10 border_box">
				<div className="fix full">
					<p className="fs_24 lh_34 font_bold text_dark_ash floatleft">Edit Product</p>
				...
				<div className="fix full mt_20 mb_20">
					<p className="fs_14 lh_20 text_dark_ash">Edit Product</p>
				</div>
				<Border />
				<form onSubmit={(e) => this.submitForm(e)}>
					<div className="fix full mt_20">
								...
								<div className="fix floatleft h_40 nine_by_ten">
									<div className="fix w_max_content">
										{this.state.images.map((image,index) => (
											<div className="single_product_thumb fix h_full w_40 floatleft pt_5 pr_5 pb_5 pl_5 border_box bt_1 br_1 bb_1 bl_1 border_solid border_ash mr_5 cursor_pointer position_relative" key={index}>
												<img src={image} className="full vertical_align_middle"/>
												<p 
												className="t_0 l_0 fs_12 font_bold display_none textcenter lh_40 cursor_pointer position_absolute full h_full" 
												onClick={(e) => this.removeProductImage(index)}
												style={{'backgroundColor' : '#4a4a4a8a'}}>
						...
						<div className="fix floatleft seven_by_ten">
							<div className="fix floatleft full">
								...
								<input type="text" className="all_border_solid_ash_1 text_field_padding_lr_5 h_30 ninty_eight_percent" name="product_name" value={this.state.product_name} onChange={(e) => this.changeInput(e)}/>
							</div>
							<div className="fix floatleft half">
								<p className="fs_14 lh_22 text_dark_ash font_bold mt_16">Select Main Category</p>
								<select className="all_border_solid_ash_1 text_field_padding_lr_5 h_30 ninty_eight_percent" name="product_main_category_list" value={this.state.product_main_category} onChange={(e) => this.changeCategory(e)}>
									{
							...
							<div className="fix floatleft half">
								...
								<select className="all_border_solid_ash_1 text_field_padding_lr_5 h_30 ninty_eight_percent" name="product_sub_category_list" value={this.state.product_sub_category} onChange={(e) => this.changeSubCategory(e)}>
									{
							...
							<div className="fix floatleft twenty_five_percent">
								...
								<input type="number" step="0.01" min="0" name="unit_price" value={this.state.unit_price} className="all_border_solid_ash_1 text_field_padding_lr_5 h_30 ninty_eight_percent" onChange={(e) => this.changeInput(e)}/>
							</div>
							<div className="fix floatleft twenty_five_percent">
								<p className="fs_14 lh_22 text_dark_ash font_bold mt_16">Offer Price</p>
								<input type="number" step="0.01" min="0" name="offer_price" value={this.state.offer_price} className="all_border_solid_ash_1 text_field_padding_lr_5 h_30 ninty_eight_percent" onChange={(e) => this.changeInput(e)}/>
							</div>
							<div className="fix floatleft twenty_five_percent">
								...
								value={this.state.offer_price_type}
					...
					<div className="fix full">
							...
							<div className="fix half floatleft">
								...
								<div className="fix sixty_percent floatleft pt_10 border_box">
									...
									<div className="fix full mt_5">
										<select 
										className="all_border_solid_ash_1 text_field_padding_lr_5 h_30 w_100 mr_10" 
										value={this.state.width_inch}
										name="width_inch" onChange={(e) => this.changeInput(e)}>
											<option value="12">12</option>
											<option value="13">13</option>
											<option value="14">14</option>
											<option value="15">15</option>
										...
										<select 
										className="all_border_solid_ash_1 text_field_padding_lr_5 h_30 w_100 mr_10" 
										value={this.state.width_8ths}
										name="width_8ths" onChange={(e) => this.changeInput(e)}>
							...
							<div className="fix half floatleft">
								...
								<div className="fix sixty_percent floatleft pt_10 border_box">
									...
									<div className="fix full mt_5">
										...
										name="height_inch" onChange={(e) => this.changeInput(e)}>
											<option value="12">12</option>
											<option value="13">13</option>
											<option value="14">14</option>
											<option value="15">15</option>
						...
						<div className="fix full mt_20">
							<div className="fix half floatleft">
								...
								<div className="fix full mt_5">
									<input type="number" step="0.01" min="0" className="w_160 h_30 pr_5 pl_5 border_box bt_1 br_1 bb_1 bl_1 border_solid border_dark_ash floatleft mr_10" placeholder="Minimum Quantity" name="minimum_quantity" value={this.state.minimum_quantity} onChange={(e) => this.changeInput(e)}/>
									<input type="number" step="0.01" min="0" className="w_160 h_30 pr_5 pl_5 border_box bt_1 br_1 bb_1 bl_1 border_solid border_dark_ash floatleft mr_10" placeholder="Maximum Quantity" name="maximum_quantity" value={this.state.maximum_quantity} onChange={(e) => this.changeInput(e)}/>
								</div>							
							</div>
							<div className="fix half floatleft">
								...
								<div className="fix full mt_5">
									...
									step="0.01" 
									min="0" 
									className="w_160 h_30 pr_5 pl_5 border_box bt_1 br_1 bb_1 bl_1 border_solid border_dark_ash floatleft mr_10" 
									placeholder="Number of Product" 
									name="total_product_number" 
									value={this.state.total_product_number}
									onChange={(e) => this.changeInput(e)}/>
								</div>
							</div>
						</div>
					...
					<div className="fix full border_box mt_10">
						<div className="fix full bg_brown2">
							...
							<div className="fix half floatright pr_10 border_box">
								<input type="checkbox" checked={(this.state.has_features == 1)} name="has_features" onChange={(e) => this.changeHasFeature(e)} className="h_20 floatright"/>
							</div>
						</div>
					</div>
					...
					<div className="fix full mt_10">
						<button type="submit" className="floatright bg_brown2 fs_14 pr_16 pl_16 font_bold text_white border_none cursor_pointer h_30">Update Product</button>
					</div>
				</form>
			</div>
		)
...
export default withRouter(Edit_product_content)
								...
								(this.state.temporary_pages.length > 0) ?
									this.state.temporary_pages.map((page, index) => (
										<tr key={index}>
											...
											<td><img src={`${process.env.MIX_APP_URL}/uploads/pages/${page.images[0].image_name}`} className="w_50 h_50"/></td>
											<td>{page.name}</td>
											<td>{page.unit_price}</td>
											<td>{page.selling_price}</td>
											<td>{page.offer_price} {page.offer_price_type}</td>
											<td>{(page.sample_available == '1') ? 'Yes' : 'No'}</td>
											<td>{(page.is_active == 0) ? 'Inactive' : 'Active'}</td>
